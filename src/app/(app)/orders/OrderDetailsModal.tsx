...
type OrderDetailsModalProps = {
  order: {
    ...
    payment: "paid" | "cod";
    address: string;
    items: Array<{ id: string; name: string; qty: string; price: string }>;
    subtotal: string;
    deliveryFee: string;
    tax: string;
    total: string;
...
const OrderDetailsModal = ({ order, isLoading = false, onClose }: OrderDetailsModalProps) => {
  return (
      ...
      <div className="relative w-full max-w-115 rounded-xl bg-white p-5 shadow-[0_24px_80px_rgba(0,0,0,0.18)] sm:p-6">
        ...
        {isLoading || !order ? (
          <div className="space-y-3 pt-8">
            <div className="inline-flex items-center gap-2 rounded-full bg-[#FAEEE8] px-3 py-1.5 text-sm font-medium text-title">
              <Loader2 className="size-4 animate-spin text-[#D94906]" />
              Loading order details...
            </div>
            <p className="text-sm text-description">Please wait.</p>
          </div>
        ) : (
